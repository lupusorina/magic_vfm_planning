# There is grass everywhere and there are sand pits. The agent has to reach the goal position.
...
# Custom imports.
from systems import TwoDimDoubleIntegratorTrue, TwoDimDoubleIntegratorNominal
from cem import CEM
...
def update_feat_rgb_and_damping_maps(xy, feature_map, rgb_map, damping_map, damping_scaling,
    ...
    for xcoord in x_coord_list:
        for ycoord in y_coord_list:
            c = np.linalg.norm(xy - [xcoord, ycoord], axis=-1) < 10
            counter_j_dino = 0
            counter_i_dino = 0
            for i in range(h):
                counter_j_dino = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--horizon_planning", type=int, default=500)
    parser.add_argument("--run_cem", type=bool, default=False)
    parser.add_argument("--cem_num_particles", type=int, default=100)
    parser.add_argument("--cem_num_iterations", type=int, default=1000)
    parser.add_argument("--cem_num_elite", type=int, default=10)
    parser.add_argument("--cem_evaluate", type=bool, default=True)
    ...
    # Systems.
    true_system = TwoDimDoubleIntegratorTrue()
        ...
        def generate_dataset_nb_steps(system,
            ...
            for i in range(1, len(nb_steps)):
                ...
                for t in range(duration):
                    ...
                    x = system.dynamics(x, us[t], dt, damping_map)
            ...
            if gen_plots == True:
                plt.savefig(args.plots_folder + str(i) + data_type + "_trajectory.pdf")
                plt.show()
    ...
    # Set x0 and xT.
    x0 = np.array([w//2 - 40, 0.0, h//2, 0.0])
    xT = np.array([w//2 + 40, 0.0, h//2, 0.0])

    fig, ax = plt.subplots(1, 2, figsize=(12, 6))
    ax[0].imshow(rgb_map)
    ax[0].plot(x0[IDX_PX], x0[IDX_PY], "o", label="start")
    ax[0].plot(xT[IDX_PX], xT[IDX_PY], "x", label="end")
    ax[0].legend()
    cax1 = ax[1].imshow(damping_map, cmap='jet')
    fig.colorbar(cax1, ax=ax[1], orientation='horizontal')
    ax[0].set(xlabel="x [m]", ylabel="y [m]")
    ax[1].set(xlabel="x [m]", ylabel="y [m]")
    plt.tight_layout()
    plt.savefig(os.path.join(args.plots_folder, "rgb_damping_map.png"))
    ...
    if args.run_cem == True:
        ...
        if args.use_NN == True:
            best_action, best_actions, reward = cem.optimize(x0, system_cem, dt, xT, damping_map=damping_map,
    ...
    if args.cem_evaluate == True:
        best_action = np.load('actions/' + 'best_action' + str(args.cem_num_iterations - 1) + '.npy')
        best_actions = np.zeros((args.cem_num_iterations, args.horizon_planning, 2))
        for i in range(args.cem_num_iterations):
            best_actions[i, :, :] = np.load('actions/' + 'best_action' + str(i) + '.npy')
    ...
    if args.cem_plot:
        ...
        for i in range(args.horizon_planning):
            next_state = true_system.dynamics(state, best_action[i], dt, damping_map)
        ...
        for i in range(args.cem_num_iterations):
            ...
            for j in range(args.horizon_planning):
                next_state = true_system.dynamics(state, best_actions[i][j], dt, damping_map)
                state = next_state.copy()
        ...
        ax.legend()
        plt.savefig(args.plots_folder + "cem_trajectory.png")

        # plot us
        fig, ax = plt.subplots()
        ax.plot(best_action[:, IDX_UX], label="u_x", linewidth=2)
        ax.plot(best_action[:, IDX_UY], label="u_y", linewidth=2)
        plt.savefig(args.plots_folder + "best_action.png")
